...
object Movie extends App {
  ...
  implicit object ParsableMovie extends Parsable[Movie] {
    /**
      ...
      * @return a Try[Movie]
      */
    def parse(w: String): Try[Movie] = Try(Movie.apply(w.split(","))) // TO BE IMPLEMENTED
  }
...
object Rating {
  ...
  match {
    case Some(Seq(Some(code), _, age)) => Rating(code, age.map(_.toInt))
    case x => throw ParseException(s"parse error in s: $s (parsed as $x)")
  }
}
